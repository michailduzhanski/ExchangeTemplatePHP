...
Yii::$app->params['templates']['assemblies'] = '<div class="row with-border" data-id="{id}" data-name="{name}"><div class="col-md-2 col-sm-2 col-xs-9">{name}</div><div class="col-md-2 col-sm-2 hidden-xs">{company_name}</div><div class="col-md-3 col-sm-2 hidden-xs">{role_name}</div><div class="col-md-3 col-sm-3 hidden-xs">{description}</div><div class="col-md-2 col-sm-3 col-xs-3"><form action="" method="post"><div id="{id}" class="abs-hidden-text">{id}</div><button type="button" data-clipboard-target="#{id}" class="btn btn-icon btn-info btn-clipboard">ID</button><a href="/' . \Yii::$app->language . '/assemblies-edit?id={id}&objectid={objectid}" class="btn btn-icon btn-primary"><i class="pe-7s-pen"></i></a><button type="button" class="btn btn-icon btn-danger" onclick="modalDelete(this)"><i class="pe-7s-trash"></i></button></form></div></div>';
Yii::$app->params['templates']['objects'] = '<option value="{id}">{name}</option>';
$json = JSONRegistryFactory::getAssembliesList(true, '0000');
...
$js = <<<JS
	...
	function updateAssemblies(){
		...
		if(currentObjectID != null){
		    ...
		    getContent('/en/drole/default/get-info', json, 'assemblies');
		}
	}
