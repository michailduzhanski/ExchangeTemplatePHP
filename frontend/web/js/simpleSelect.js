...
(function($){
    $.fn.simpleSelect = function(options){
        ...
        function buildSelect(terms) {
            var selectOptions = '<option value="' + options.defaultSelected['id'] + '">' + options.defaultSelected['title'] + '</option>';
            ...
            if (!options.terms.length && !!$childs.length) {
                for (var i = 0; i < $childs.length; i++) {
                    if ($childs[i].hasAttribute('data-defaultSelected') )
                        continue;
                    options.terms.push({"id":$childs[i].value, "title":$childs[i].innerText});
                }
                selectOptions = '<option value="'+$childs[0].value+'">' + $childs[0].innerText + '</option>'
                $container.find('.selected').text($childs[0].innerText);
            }

            for (var i = 0; i < terms.length; i++) {
                selectOptions += '<option value="' + terms[i]['id'] + '">' + terms[i]['title'] + '</option>';
            }
        ...
        function searchTerm(q) {
            ...
            for (var i = 0; i < terms.length; i++) {
                if (regex.test(terms[i]['title'].toUpperCase()))
                    termsFound.push(terms[i]['title']);
            }
